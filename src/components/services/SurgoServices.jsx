...
export default function SurgoServices({ services: input }) {
  ...
  const [expanded, setExpanded] = useState(false); // NEW: controls the white slab
  const [atEnd, setAtEnd] = useState(false);
  const scrollRef = useRef(null);

  // Optional auto-rotation (paused when expanded)
  useEffect(() => {
    if (expanded) return; // pause rotation while panel is open
  ...
  // Lock page scroll while expanded (mobile UX)
  useEffect(() => {
    if (expanded) {
  ...
  // Detect scroll position to hide arrow at end
  const handleScroll = () => {
  ...
  return (
    <section className="max-w-[1400px] mx-auto px-4 md:px-8 py-16 md:py-24 text-white">
      ...
      {/* Featured Card (image + “peek” info box trigger) */}
<motion.article
...
>
  <div className="relative rounded-3xl overflow-hidden bg-black/60">
    {/* Media surface — no onClick here; the box is the trigger */}
    <div className="relative aspect-[16/9]">
      <PosterMedia image={active.image} icon={active.icon} title={active.title} />
      <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/20 to-transparent" />
    </div>

    {/* REMOVE the old title/desc strip entirely (no collapsed text) */}

    {/* “Peek” info box trigger (Title + Subtitle + expand icon) */}
    {!expanded && (
      <motion.button
      ...
      >
        <div className="flex-1 text-left">
          <div className="text-base md:text-lg font-semibold leading-tight">{active.title}</div>
    ...
    {/* EXPANDED WHITE SLAB (unchanged behavior) */}
    <motion.div
      initial={false}
      animate={expanded ? { opacity: 1, y: 0, scale: 1 } : { opacity: 0, y: 20, scale: 0.995 }}
    ...
    >
      ...
      <div className="p-5 md:p-7">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mt-4">
          <div>
            <div className="text-xs tracking-widest text-neutral-500 uppercase">How we help</div>
            <p className="mt-1 text-sm md:text-base text-neutral-800">
              {active.help ?? active.desc}
            </p>
          </div>
          <div>
            <div className="text-xs tracking-widest text-neutral-500 uppercase">What you get</div>
            <p className="mt-1 text-sm md:text-base text-neutral-800">
              {active.outcome ?? "Clear creative direction, production plan, and deliverables tailored to your launch."}
            </p>
          </div>
        </div>
        ...
        <div className="mt-5 md:mt-6">
          ...
          >
            ...
            <svg viewBox="0 0 24 24" className="w-4 h-4 -mr-0.5" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M5 12h14M13 5l7 7-7 7" />
            </svg>
          </a>
        </div>
      ...
      {/* Thumbnails Row (kept same, bigger cards + scroll indicator) */}
          ...
          {services.map((s, i) => (
            <motion.div
              ...
              className={`shrink-0 w-[280px] rounded-2xl border border-white/10 cursor-pointer transition ${
                activeIndex === i ? "bg-white/10 border-white/30" : "bg-white/[0.02] hover:bg-white/[0.04]"
              }`}
              title={s.title}
            >
...
function CTA({ className = "" }) {
  return (
    <div className={"rounded-3xl p-[2px] bg-gradient-to-r from-white/15 via-white/8 to-white/15 " + className}>
      <div className="rounded-3xl bg-gradient-to-r from-white/[0.06] to-white/[0.02] p-8 md:p-12 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div>
          <h3 className="text-2xl md:text-3xl font-semibold">Ready to make it cinematic?</h3>
          <p className="text-white/70 mt-2 max-w-xl">
            Brief us in two minutes. We’ll respond with an approach and timeline.
          </p>
        </div>
        <a href="/contact" className="group inline-flex items-center gap-2 rounded-xl bg-white text-black px-5 py-3 font-medium hover:bg-zinc-100 transition">
          Book a Call
          <svg viewBox="0 0 24 24" className="w-4 h-4 -mr-1 transition group-hover:translate-x-0.5" fill="none" stroke="currentColor" strokeWidth="2">
            <path d="M5 12h14M13 5l7 7-7 7" />
          </svg>
        </a>
      </div>
...
/* ---------- Default data (you can add help/outcome/cta per item) ---------- */
const DEFAULT_SERVICES = [
  {
    ...
    outcome: "Locked narrative, creative references, and a day‑by‑day plan that saves time on set.",
  ...
  },
  {
    ...
    outcome: "Broadcast‑safe stems and mixes (stereo/5.1) that make your cut feel bigger.",
  ...
  },
  {
    ...
    outcome: "Hero film + short‑form variants that hook in 3s and keep viewers to the end.",
