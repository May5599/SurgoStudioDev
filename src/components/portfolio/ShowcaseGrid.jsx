...
export default function CardShowcase() {
  ...
  <div
    className="flex gap-6 overflow-x-auto no-scrollbar snap-x snap-mandatory scroll-smooth w-full"
  >
    {showcaseItems.map((item, i) => {
      ...
      return (
        <motion.article
          ...
          animate={{
            scale: isActive ? 1.1 : 0.95, // active card pops, others shrink slightly
            y: isActive ? -20 : 0,        // lift active card upward
            boxShadow: isActive
              ? "0 20px 60px rgba(0,0,0,0.6)"
              : "0 8px 20px rgba(0,0,0,0.3)",
          }}
          transition={{ duration: 0.5, ease: "easeOut" }}
          whileHover={{ scale: 1.12 }} // extra pop on hover
        >
          <img
            src={item.image}
            alt={item.title}
