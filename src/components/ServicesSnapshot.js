...
export default function ServicesSnapshotClient() {
  ...
  return (
    <section
      className="relative py-24 px-6 bg-black/95 text-white overflow-hidden"
      aria-label="Creative services offered by Surgo Studios"
    >
      <div className="max-w-7xl mx-auto space-y-16 text-center relative z-10">
        {/* Section Heading */}
        <div className="space-y-4">
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-mozilla font-bold drop-shadow-md">
          ...
          {services.map((service, i) => {
            ...
            return (
              <motion.div
                ...
                className="relative group p-6 rounded-3xl border border-white/10 bg-white/5 backdrop-blur-md transition-all duration-300 hover:scale-[1.03] hover:shadow-[0_0_30px_rgba(255,255,255,0.08)]"
              >
                {isHovered && (
                  <motion.div
                    layoutId="glow"
      ...
      <div className="absolute top-0 left-0 w-full h-full z-0 pointer-events-none">
        <div className="absolute w-96 h-96 top-[-20%] left-[-10%] rounded-full bg-gradient-to-r from-yellow-500/20 to-pink-500/20 blur-3xl animate-pulse-slow" />
        <div className="absolute w-64 h-64 bottom-[-15%] right-[-5%] rounded-full bg-gradient-to-r from-purple-500/20 to-blue-500/20 blur-3xl animate-pulse-slow delay-1000" />
      </div>
